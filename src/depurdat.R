#  quiz? se pueda arreglar tratando esas series como si fueran diarias...
#  nref[3]=nref[2] maxdif=mxdif

#depurdat.R.- Depuraci?n y homogeneizaci?n de series climatol?gicas.
...
#- dahgrid.- Generaci?n de grids de datos homogeneizados.
dahgrid <- function(varcli, anyi, anyf, anyip=anyi, anyfp=anyf, grid,
  mh=FALSE, std=NA, ini=NA, obsonly=TRUE) {
  #anyip: a?o inicial de referencia (para el c?lculo de las anomal?as)
  #anyfp: a?o final de referencia
  #grid: grid a interpolar, de clase SpatialPixel
  #mh: Si TRUE, leer datos mensuales de la homogeneizaci?n diaria (*-mh_*.dat)
  ...
  #     y entonces puede especificarse aqu?. (Por defecto, 1 de enero de anyi))
  ...
  if(!mh) {
    fbas <- sprintf('%s_%d-%d',varcli,anyi,anyf) #ra?z nombres de fichero
    load(sprintf('%s.rda',fbas))
  } else { #leer los datos mensuales *-mh_*.dat (generados por dd2m)
    fbas <- sprintf('%s-mh_%d-%d',varcli,anyi,anyf) #ra?z nombres de fichero
  ...
  dim(dah) <- c(nd,ne) #conversi?n a 2 dimensiones
  dim(dat) <- c(nd,nei) #conversi?n a 2 dimensiones
  #- seleccionar las series de los fragmentos m?s largos
  sel <- tapply(est.c[,6],est.c[,7],which.max)
  ksel <- rep(NA,nei)
  for(k in 1:nei) ksel[k] <- which(est.c[,7]==k)[sel[k]]
  #- retener solo los datos homogeneizados (dah) de las subseries m?s largas
  dah <- dah[,ksel]
  #- calcular sus medias y desviaciones t?picas en el periodo escogido
  ...
  #- interpolar las medias (y desv. t?picas), y grabarlas en NetCDF
  ...
  if(std>2) {
    ...
    names(df) <- c('x','y','z')
    sp::coordinates(df) <- ~x+y
    s <- gstat::idw(z~1, df, grid, debug.level=0) #desv. t?picas interpoladas
  ...
  for(k in 1:nd) {
    ...
    names(df) <- c('x','y','z')
    sp::coordinates(df) <- ~x+y
    z <- gstat::idw(z~1, df, grid, debug.level=0) #desv. t?picas interpoladas
...
#- dahstat.- Estad?sticas de datos homogeneizados.
...
#stat="me"(valores medios), "mdn"(medianas), "max"(m?ximos), "min"(m?nimos),
#  "std"(desv.t?p.), "q"(cuantiles), "tnd"(tendencias), "series"(series
#  secuenciales, en dos ?nicos ficheros), "mseries"(series mensuales, dos
#  ficheros por estaci?n)
#  stat='tnd' genera tambi?n los p-valores, en *.pval
#ndc=no. de decimales (con prioridad sobre el ndec guardado en el *.rda)
#vala= 0(ninguno), 1(suma), 2(media), 3(m?ximo), 4(m?nimo)
#cod=lista de c?digos de las estaciones a listar
#mnpd=M?nimo porcentaje de datos originales
#mxsh=M?ximo SNHT
#last=Listar solo las series del ?ltimo periodo homog?neo
#long=Listar solo las series con mayor fragmento original
#lsnh=Listar solo las series con menor valor SNHT
#lerr=Listar solo las series con menor error cuadr?tico medio
#relref=Listar tambi?n las series de referencia confiables (*Cod)
#mh=Si TRUE, leer datos mensuales de la homogeneizaci?n diaria (*-mh_*.dat)
#pernys=No. de a?os sobre los que se expresan las tendencias (100 por defecto)
#estcol=Columnas de est.c seleccionadas para el listado
#Los par?metros sep, dec y eol permite personalizar los formatos de salida
...
#x=vector de fechas. (Se obtiene del fichero *.rda).
  ...
  #funci?n elegida para el c?lculo de los valores mensuales:
  fun <- c("mean","median","max","min","sd","quantile")[which(c("me","mdn","max","min","std","q","tnd")==stat)]
  #- si no se reconoce la opci?n stat, terminar aqu?
  ...
  if(mh) { #leer los datos mensuales *-mh_*.dat (generados por dd2m)
    ...
    if(nm>0) na <- nd/nm else na <- nd #no. de a?os o de datos (sub)diarios
    if(nm<2) dim(dah) <- c(1,na,ne)
  }
  if(nm<2 | stat=='series' | stat=='mseries') vala <- 0 #valor anual innecesario
  else {
    if(vala<0 | vala>4) vala <- 2 #valor medio en caso de vala err?neo
    funa <- c("sum","mean","max","min")[vala] #funci?n para el valor anual
  }
  ...
  #seleccionar las estaciones con un m?nimo de mnpd % de datos originales:
  if(mnpd>0) esel <- esel & est.c[,6]>=mnpd #vector de la selecci?n
  #seleccionar las estaciones con un SNHT menor o igual a mxsh:
  if(mxsh>0) esel <- esel & est.c[,9]<=mxsh #vector de la selecci?n
  if(last & ne>nei) esel[(nei+1):ne] <- FALSE #solo ?ltimos fragmentos?
  else if(long | lsnh | lerr) {
    lsel <- rep(TRUE,length(esel)) #inicializar vector
    for(ko in 1:nei) { #para cada estaci?n original
      kest <- which(est.c[,7]==ko) #series de la misma estaci?n ko
      if(length(kest)>1) { #si hay m?s de un fragmento...
        if(long) ksel <- which.max(est.c[kest,6]) #mayor % de datos originales
        else if(lsnh) ksel <- which.min(est.c[kest,9]) #menor SNHT
        else ksel <- which.min(est.c[kest,10]) #menor error cuadr?tico medio
        lsel[kest[-ksel]] <- FALSE #selecci?n deseada
      }
    }
    esel <- esel & lsel
  }
  ...
  iest <- est.c[,7] #?ndice de las correspondientes estaciones originales
  #- if(stat=="[m]series"), listar las series y sus flags en formato CSV
  if(stat=='series') { #series secuenciales, en dos ?nicos ficheros:
    dim(dah) <- c(nd,ne); dim(dat) <- c(nd,length(dat)/nd)
    #comparaci?n datos homogeneizados con originales (no usar '=='!):
  ...
  if(stat=='mseries') { #series mensuales, cada estaci?n dos ficheros:
    xa <- anyi:anyf #vector de a?os
    for(kest in 1:ne) { #para cada estaci?n seleccionada
  ...
  #- calcular la posici?n de los datos del periodo solicitado:
  if(nm>0) xk <- (anyip-anyi+1):(anyfp-anyi+1)
  else { #posici?n de las fechas solicitadas:
    datef <- as.Date(sprintf('%s-12-31',anyfp)) #fecha final (m?xima)
  ...
  #- else, aplicar la funci?n deseada al periodo escogido
  else { #aplicar la funci?n deseada al periodo escogido
    for(i in 1:ne) {
      if(nm<2) {
...
#ninny= Minimum number of years with data to subset.
  ...
  fbas <- sprintf('%s_%d-%d',varcli,anyi,anyf) #ra?z nombres de fichero
  fbas2 <- sprintf('%s_%d-%d',varcli,anyis,anyfs) #ra?z nombres ficheros salida
  ...
  numdat <- length(dat) #no. de datos le?dos
  nd <- numdat/ne #no. de datos por estaci?n
  dim(dat) <- c(nd,ne) #conversi?n de vector a matriz
  #calcular no. de datos por a?o y estaci?n:
  z <- nd/na
  if(z>=1) nm <- ceiling(z)
  ...
  if(nas < na) { #subset a subperiod of data
    xa <- strftime(x,"%Y") #a?os de cada dato
...
#- dd2m.- C?lculo de valores mensuales a partir de datos diarios.
dd2m <- function(varcli, anyi, anyf, anyip=anyi, anyfp=anyf, ndec=1, suf=NA,
  valm=2, namax=10, na.strings="NA", homog=FALSE, ini=NA) {
  #suf: sufijo opcional a a?adir al nombre de la variable para leer los datos.
  #valm: Valor mensual (1=suma, 2=media, 3=m?ximo, 4=m?nimo)
  #namax: M?ximo no. permitido de datos diarios ausentes originalmente
  #homog: Usar datos ya homogeneizados? (poner homog=TRUE)
  #ini: Fecha inicial. Si es NA se supone que es el 1 de enero de anyi
  fbas <- sprintf('%s_%d-%d',varcli,anyi,anyf) #ra?z nombres de fichero
  if(is.na(suf)) fntr <- fbas #ra?z nombres ficheros de entrada
  ...
  if(homog) {
    ...
    nd <- length(dah)/ne #no. de datos por estaci?n
    dim(dah) <- c(nd,ne)
    na <- anyf-anyi+1 #no. de a?os
    nm <- nd/na #no. de "meses" (no. de datos por a?o y estaci?n)
  ...
  fun <- c("sum","mean","max","min")[valm] #funci?n para el valor mensual
  na <- anyfp-anyip+1 #no. de a?os
  dm <- matrix(NA,na*12,ne) #datos mensuales
  for(ie in 1:ne) { #para cada estaci?n
    cat(' ',ie)
...
#- homogen.- homogeneizaci?n autom?tica de un conjunto de series de datos.
...
metad=FALSE, sufbrk='m', tinc=NA, tz='UTC', cex=1.2, verb=TRUE) {
  #varcli: variable clim?tica (acr?nimo usado)
  #anyi: a?o inicial
  #anyf: a?o final
  #suf: sufijo opcional a a?adir al nombre de la variable para leer los datos.
  #nm: n?mero de meses. (Si no se fija, se calcula por el no. de datos)
  #nref: no. (m?ximo) de estaciones de referencia
  #dz.max: l?mite superior de tolerancia de anomal?as 
  #dz.min: l?mite inferior de tolerancia de anomal?as
  ...
  #trf: Transformar los datos? (0:no transformar; 1:log(x+1); >1:ra?z trf)
  #mndat: M?nimo no. de datos para fragentar las series
  #gp: Par?metro de gr?ficos. 0=ninguno; 1=anomal?as globales e histogramas;
  #    2=id+gr?ficos mensuales de diagn?stico; 3=id+gr?ficos de medias anuales
  #    m?viles y correcciones; 4=id., pero con sumas anuales m?viles
  ...
  #vmin, vmax: rango de valores permitidos en la variable clim?tica.
  #nclust: no. m?ximo de estaciones a usar en el an?lisis de agrupamiento
  ...
  #   y x,y en km. Tambi?n sirve para sobreponderar z, o para hallar las
  #   distancias ?nicamente en el plano horizontal (wz=0).
  #grdcol: color de las ret?culas de los gr?ficos.
  #mapcol: color del mapa de fondo.
  #hires: mapa de fondo en alta resoluci?n. (Poner a FALSE si no hace falta).
  #expl: Pasada exploratoria? (FALSE por defecto).
  #metad: Usar metadatos? En ese caso se fragmentar?n las series en los lugares
  #   indicados en *brk.csv y solo se rellenar?n lagunas. (FALSE por defecto).
  #sufbrk: sufijo a a?adir al nombre de la variable para leer los metadatos.
  ...
  #cex: Character expansion factor para etiquetas y textos en los gr?ficos.
  #verb: Ver mensajes del proceso por pantalla (adem?s de en el fichero *.txt).
  # -----------------------------------------------------------------  
  #- inicializaciones
  ...
  #establecer maxdif en funci?n de la precisi?n elegida:
  ...
  #- abrir fichero de bit?cora y escribir cabecera
  ...
  cat("\nParameters:")
  arg <- names(formals()) #lista de los argumentos de la funci?n
  for(i in 1:length(arg)) {
    cat(" ",arg[i],"=",sep="")
    cat(eval(as.symbol(arg[i])),sep=",")
  }
  cat("\n\n")
  #par?metros que han de tener 3 valores:
  ...
  ne <- nrow(est.c) #no. de estaciones
  #si los c?digos continenen guiones, cambiarlos a guiones bajos:
  z <- gsub('-','_',est.c[,5])
  zn <- sum(z != est.c[,5])
  if(zn>0) {
  ...
  #comprobar si hay c?digos duplicados:
  ...
  #comprobar si las coordenadas est?n en grados:
...
# }
  ...
  dim(dat) <- c(nd,ne) #conversi?n de vector a matriz
  na <- anyf-anyi+1 #no. de a?os
  nsy <- rep(0,na)   #no. de saltos por a?o
  if(is.na(nm)) { #calcular no. de datos por a?o y estaci?n:
    z <- nd/na
    if(z>=1) nm <- ceiling(z)
    if(nm > 12) nm <- 0 #datos diarios
    else if(!nm%in%c(1,2,3,4,6,12)) {
  ...
  #comprobar si los a?os est?n completos:
  ...
  #establecer valor de mndat, si no se especific?:
  if(is.na(mndat)) { 
    if(nm<=0) mndat <- swa/2 else mndat <- max(5,nm)
  }
  #- comprobar si hay valores fuera del rango permitido
  if(!is.na(vmin)) { #hay valores inferiores al m?nimo permitido?
    n <- sum(dat<vmin,na.rm=TRUE)
    if(n) {
      dat[dat<vmin] <- vmin #correcci?n de los valores err?neos
      cat(n,"data forced to",vmin,":\n")
    }
  }
  if(!is.na(vmax)) { #hay valores superiores al m?ximo permitido?
    n <- sum(dat>vmax,na.rm=TRUE)
    if(n) {
      dat[dat>vmax] <- vmax #correcci?n de los valores err?neos
      cat(n,"data forced to",vmax,":\n")
    }
  }
  dat.o <- dat #copia de los datos originales
  if(nd<100) lw=3 #anchura de las barras de anomal?as
  else if(nd<300) lw=2
  else lw=1
  #- generar vector temporal x
  if(is.na(ini)) ini <- sprintf('%d-01-01',anyi) #fecha inicial por defecto
  ...
  #- si hay alg?n t?rmino sin datos, emitir aviso y terminar
  numdat <- apply(!is.na(dat),1,sum) #no. de datos de cada t?rmino
  if(!min(numdat)) {
  ...
  nsp <- rep(0,nei)  #no. de cortes de cada estaci?n original
  iest <- 1:ne   #?ndice se?alando la serie original de cada subserie
  outan <- matrix(NA,nd,ne) #anomal?as de los outliers
  #- if(gp>0), generar gr?ficos iniciales
  if(gp>0) {
    #activar salida gr?fica a documento pdf, con r?tulo inicial:
    ...
    #de estaciones es muy grande, el gr?fico ocupa mucho espacio):
    if(ne<=5*nclust) {
      image(x,1:ne,dat,xlab='Time',ylab='Stations',main=paste(varcli,'data availability'),col=4)
      grid(col=grdcol)
    } else cat(sprintf('\nMore than 5*%d stations: Per station data availability graph skipped\n\n',nclust))
    #no. de datos de cada t?rmino
    ...
    #si hay alg?n t?rmino sin datos no podremos continuar:
    if(!min(numdat)) {
      cat("At least one term has missing data in all stations! (See the PDF graph)\n")
    }
    #boxplots de los datos de cada estaci?n:
    if(nm>1 & acomp) { #boxplots para cada nm (mensuales, etc)
      ...
      for(me in 1:nm) { #para cada mes
        z <- data.frame(dat[me,,])
        names(z) <- 1:ne
        #etiqueta del mes (si nm!=12, poner solo el n?mero):
    ...
    else if(ne<=nclust) { #un solo gr?fico con los boxplots de cada estaci?n
      z <- data.frame(dat)
      names(z) <- 1:ne
  ...
  if(gp>0) { #continuamos con los gr?ficos iniciales
    #histograma de todos los datos (distribuci?n quasi-normal?)
    ...
    #(si hay m?s de nclust estaciones, solo de una muestra aleatoria de nclust)
    ...
    for(i in 1:(nec-1)) {
      for(j in (i+1):nec) {
        ...
        d2 <- dx*dx+dy*dy+dz*dz #distancia cuadr?tica
        est.d[i,j] <- sqrt(d2) #distancia
        est.d[j,i] <- est.d[i,j]  #matriz sim?trica
      }
    }
    ...
    #eliminar |r|==1 (debidos a estaciones con solo 2 datos en com?n):
    ...
    if(ne>2) {  #dendrograma de las estaciones
      ...
      if(!sum(is.na(dism))) {
        ...
        #clasificaci?n de las estaciones hasta en un m?ximo de 9 grupos:
...
#       else { maps::map.axes(); mtext(main,3,line=1,cex=1.7) }
      ...
      if(ne>99) { #dibujar s?mbolos si hay m?s de 99 estaciones
        #dibujar primero en negro las estaciones que no est?n en la muestra:
        points(est.c[-splc,1:2],pch='+',cex=.5)
        #y ahora las estaciones de la muestra, con sus colores:
        points(est.c[splc,1:2],col=col,pch=ct)
      } else { #hasta nclust estaciones, poner el n?mero
  ...
  #- if(gp==1), terminar (solo se deseaban los gr?ficos iniciales)
  if(gp==1) {
    par(old.par)
    graphics.off() #volcar el buffer del ?ltimo gr?fico
    cat("\nOnly the initial exploratory graphics were demanded.\nSee them in ",varcli,"_",anyi,"-",anyf,".pdf\n",sep="")
    sink() #cerrar el fichero de bit?cora
    return(invisible())
  }
  #  Proceso de homogeneizaci?n en tres etapas:
  #  1) cortes por SNHT en ventanas m?viles
  ...
  #- c?lculo de las matrices de distancias y rangos de proximidad
  ...
  for(i in 1:(ne-1)) {
    ...
    for(j in (i+1):ne) {
      ...
      d2 <- dx*dx+dy*dy+dz*dz #distancia cuadr?tica
      #evitar autocorrecci?n en series con las mismas coordenadas: 
      if(d2==0 & iest[i]!=iest[j]) est.d[i,j] <- 0.01 #evitar autocorrecci?n 
  ...
  refhom <- substr(est.c[,4],1,1)=='*' #referencias homog?neas
  #- Estima inicial de medias y desv. t?p. por diferencias o proporciones
  ...
  if(std==3) {
    refstd <- sd(datmed) #desv. t?pica global de referencia
    dat.s <- apply(dat,2,sd,na.rm=TRUE) #desv. t?p. de partida
  }
  switch(std,
  ...
  if(metad) {
    ...
    if(nbrk<1) cat('No break-points in the metadata file.\n')
      ...
      for(kb in 1:nbrk) { #para cada break:
        i <- match(brk[kb,1],est.c[,4]) #estaci?n a cortar
        if(is.na(i)) {
          cat(sprintf('\nCode %s not found in station list; break skipped',brk[kb,1]))
          next
        }
        kp <- match(brk[kb,2],x) #posici?n de corte
        ...
        if(sum(!is.na(dat[1:(kp-1),i])) < mndat) {
          ...
          iest <- c(iest,iest[i]) #a?adir ?ndice a la serie original
          nsp[iest[i]] <- nsp[iest[i]]+1 #y tambi?n su no. de saltos
          if(nm>0) { #contar no. de saltos por a?o
            z <- 1 + floor((kp-1)/nm) #t?rmino anual del salto
            nsy[z] <- nsy[z] + 1 #no. de saltos por a?o
          }
      ...
      cat(ne,"\n\n")
      refhom <- substr(est.c[,4],1,1)=='*' #actualizar referencias homog?neas
    }
  ...
  for (ks in 1:3) { #para cada etapa:
    ...
    if(wd[ks]>0) { #pesos diferentes de 1
      ...
      for(i in 1:(nei-1)) {
        for(j in (i+1):nei) {
          ...
          est.w[j,i] <- est.w[i,j]  #matriz sim?trica
        }
      }
      cat(' (done)\n\n')
    }
    #- if(gp>0), pintar r?tulo separador de niveles
    if(gp>0) {
      par(old.par)
      plot(-1:1,-1:1,type="n",xaxt="n",yaxt="n",bty="n",xlab="",ylab="")
    ...
    repeat {
      #---------- C?lculo de anomal?as y eliminaci?n de datos an?malos:
      ...
      anom <- matrix(NA,nd,ne) #anomal?as
      sanom <- matrix(NA,nd,ne) #anomal?as estandarizadas
      mindist <- matrix(NA,nd,ne) #distancias m?nimas
      nrefs <- matrix(NA,nd,ne) #no. de referencias
      used <- matrix(FALSE,ne,ne) #flags de estaciones usadas
      #copia de trabajo de los datos:
      dat.d <- dat
      #- si hay alg?n t?rmino sin ning?n dato, avisar y terminar
      ...
      if(nmin<mndat) {
        ...
        iest[z] <- 0 #anular los ?ndices de estaci?n original
      }
      dat.na <- is.na(dat.d) #?ndice de datos ausentes
      #usar las medias y desviaciones t?picas anteriores si existen:
      if(exists('dat.m0')) {
        dat.m <- dat.m0
      ...
      repeat {
        ...
        for(i in 1:ne) { #para cada estaci?n
          if(!iest[i]) next #estaci?n borrada
          if(refhom[i]) next #saltar estaciones confiables
          ik <- iest[i] #?ndice de referencia estaci?n inicial
          for(j in 1:nd) { #para cada dato
            se <- 0
            sw <- 0
            nr <- 0
            for(ir in 1:nei) { #para cada estaci?n (posible referencia)
              kr <- est.p[ik,ir]
              krf <- which(iest==kr) #fragmentos de la referencia
              k <- which(!dat.na[j,krf]) #cu?l tiene dato observado?
              if(length(k)!=1) next #ning?n fragmento con dato
              k <- krf[k] #?ndice del fragmento con dato
              if(i==k) next #es la misma estaci?n
              nr <- nr+1 #no. de referencias
              used[i,k] <- TRUE #marca de estaci?n usada
              #distancia m?nima (distancia al dato m?s pr?ximo):
              if(nr==1) mindist[j,i] <- max(est.d[ik,kr],1)
              w <- est.w[ik,kr]
              se <- se + w * dat.z[j,k]
              sw <- sw + w
              if(nr>=nrefk) break #si no. m?x. de referencias, terminar
            }
            if(!nr) { #sin referencia!
              ...
              #no permitir datos negativos si std=2 (precipitaci?n, etc):
              if(std==2 & se<0) se <- 0
        ...
        #valores calculados por desestandarizaci?n de dat.e:
        switch(std,
        ...
        #- c?lculo de anomal?as (anom, sanom) y eliminaci?n de outliers
        anom <- dat.z-dat.e #anomal?as
        anom[dat.na] <- NA  #no arrastrar anomal?as de datos rellenados
        #estandarizar las anomal?as:
        anomm <- apply(anom,2,mean,na.rm=TRUE) #anomal?as medias
        anoms <- apply(anom,2,sd,na.rm=TRUE) #desv. t?picas de las anomal?as
        ...
        if(!expl & dz.maxk>.1) { #eliminar outliers
          ...
          if(nelim>0) { #eliminar los datos originales an?malos
            #listado de los datos a eliminar:
            for(i in 1:ne) {
              for(j in 1:nd) if(elim[j,i] & !is.na(oneref[j,i])) {
                outan[j,iest[i]] <- sanom[j,i] #guardar la anomal?a del outlier
                ...
                #no eliminar si solo ten?an una referencia!:
            ...
            dat[elim] <- NA #eliminaci?n de los datos an?malos
            dat.na[elim] <- TRUE #actualizaci?n ?ndice de datos ausentes
          }
        ...
        if(ite>1) {
          maxddif <- max(abs(dat.d-dat.d0),na.rm=TRUE) #m?x. dat. dif.
          maxdif3 <- 3*maxddif #control de convergencia
          kmaxdif <- which.max(abs(dat.d-dat.d0)) #posici?n m?x. dat. dif.
          kmaxest <- ceiling(kmaxdif/nd) #estaci?n m?x. dat. dif.
        ...
        if(!aref) break
        #- if(ite>1), si los datos ya no var?an, break
        if(ite>1) {
      ...
      if(std==3) dat.s0 <- dat.s #copia de las desv. t?picas
      #restablecer valores de oneref:
      oneref[is.na(oneref)] <- TRUE
      #- if(aref==TRUE), repetir relleno de lagunas con autocorrecci?n
      if(aref==TRUE) { # de las series fragmentadas:
        ...
        for(i in 1:ne) { #para cada estaci?n
          ...
          for(j in 1:nd) { #para cada dato
            ...
            for(ir in 1:nei) { #para cada estaci?n (posible referencia)
              ...
              se <- se + w * dat.z[j,k]
              sw <- sw + w
              #si no. m?x. de referencias, o autoreferencia, terminar:
        ...
        #- c?lculo final de dat.m|s|z
        dat.m <- apply(dat.d,2,mean,na.rm=TRUE) #medias
        if(std==3) dat.s <- apply(dat.d,2,sd,na.rm=TRUE) #desv. t?picas
      ...
      #- calcular los valores finales de las anomal?as (anom, sanom)
      anom <- dat.z-dat.e #anomal?as
      anom[dat.na] <- NA  #no arrastrar anomal?as de datos rellenados!
      anomm <- apply(anom,2,mean,na.rm=TRUE) #anomal?as medias
      anoms <- apply(anom,2,sd,na.rm=TRUE) #desv. t?picas de las anomal?as
      sanom <- scale(anom,center=anomm,scale=anoms) #anomal?as estandarizadas
      #- ----------- An?lisis de saltos en la media (binary split):
      #- if(ks>2) break (en la ?ltima etapa, solo relleno final de lagunas)
      if(ks>2) break
      #analizar los saltos en la media de las series, cort?ndolas cuando
      #el m?ximo snht del test supere el umbral (snht1):
      nn <- 0 #inic. no. de nuevas estaciones
      tVx <- rep(NA,ne) #m?ximos valores del shift test (por estaci?n)
      kpx <- rep(NA,ne) #posiciones de los m?ximos tV (por estaci?n)
      splt <- rep(0,ne)  #tV con que se cortaron las estaciones
      modif <- FALSE #inicializaci?n modificaci?n series
      cat("\nPerforming shift analysis on the",ne,"series...\n")
      for(i in 1:ne) { #an?lisis de saltos en la media para cada estaci?n
        if(refhom[i]) next #no analizar las series confiables
        y <- sanom[,i] #anomal?as estandarizadas de la estaci?n
        if(ks==1) { #an?lisis de saltos en ventanas m?viles
          st <- snhtw(y,swa) #prueba SNHT en ventanas solapadas
          if(st[1]>0) tVx[i] <- st[1] else tVx[i] <- NA
          kpx[i] <- st[2]
        }
        else { #an?lisis de saltos en toda la serie
          st <- snht(y)
          if(sum(!is.na(st))>0) { 
      ...
      #- cortar las series cuyo snht m?ximo supere el umbral, de mayor a menor
      #  siempre que no se hayan usado series reci?n cortadas con snht similar
      #m?ximo tVx de todas las estaciones
      if(sum(!is.na(tVx))==0) tVxx <- 0 else tVxx <- max(tVx,na.rm=TRUE)
      while(tVxx > snht1) {
        i <- which.max(tVx) #estaci?n con el m?ximo snht
        #si i us? referencias cortadas con un snht demasiado grande, iniciar
        #una nueva iteraci?n:
        if(max(splt[used[i,]])>tVxx*(1+tol*min(nr,sum(used[i,])))) break
        kp <- kpx[i] #posici?n del tVx en la estaci?n i
        if(oneref[kp,i] & !force & nrefk>1) { #no cortar con una sola referencia
          tVx[i] <- -1 #pasar el tVx de esta estaci?n a -1
          tVxx <- max(tVx,na.rm=TRUE) #m?ximo tVx de las estaciones restantes
          next
        }
        ...
        write(sprintf('%s,%s,%.1f',est.c[iest[i],4],format(x[kp]),tVx[i]),
          Fbrk,ncolumns=3)
        #gr?fico de anomal?as con la posici?n del corte:
        if(gp>1) {
          y <- sanom[,i] #vector de anomal?as de la estaci?n
        ...
        #contar no. de saltos por a?o
        z <- as.integer(strftime(x[kp],"%Y"))-anyi+1 #t?rmino anual del salto
        nsy[z] <- nsy[z] + 1 #no. de saltos por a?o
        ...
        splt[i] <- tVx[i] #tV de corte de la estaci?n i
        tVx[i] <- 0 #anular el tVx de esta estaci?n
        tVxx <- max(tVx,na.rm=TRUE) #m?ximo tVx de las estaciones restantes
      }
      if(nn) {
        cat("\n\nUpdate number of series: ",ne,"+",nn,"= ")
      ...
      if(!nn & !modif) {
        if(gp>1) {
          #histograma de m?ximos tV, globales (sin 0's, que no son reales):
          ...
          if(ks==2 | snht2<1) {
            #histograma de no. de cortes por estaci?n:
            ...
            #frecuencias de fragmentaci?n por a?os:
  ...
  #------------ Fin de las tres fases de la homogeneizaci?n ---------
  ...
  #- gr?ficos de anomal?as de las series homogeneizadas
  #  (con tVx m?ximos, ordenados por series originales):
  tVx <- rep(NA,ne) #(guardaremos los m?ximos tV finales)
  snhx <- rep(NA,ne) #(guardaremos los m?ximos SNHT finales)
  for(io in 1:nei) { #para cada serie original
    wi <- which(iest==io) #estaciones derivadas de la estaci?n io
    lwi <- length(wi)
    if(!lwi) next #(estaci?n totalmente borrada!)
    for(i in wi) { #para cada serie derivada de la original
      ...
      #aplicar SNHTw y marcar su tV m?ximo (si >=1):
      st <- snhtw(y,swa); tVx[i] <- st[1]; zz <- floor(st[1])
      if(zz) {
        kp <- st[2]
        if(gp>1) {
          lines(rep(x[kp],2),c(-5,4.8),col=verde,lty=2) #marca m?ximo SNHTw
          text(x[kp],5,zz,col=verde) #valor
        }
      }
      #aplicar SNHT y marcar su m?ximo:
      st <- snht(y)
      if(sum(!is.na(st))>0) {
        kp <- which.max(st)
        ...
        zz <- floor(snhx[i])
        if(gp>1) {
          lines(rep(x[kp],2),c(-5,4.8),lty=4) #marca m?ximo SNHT
  ...
  if(trf==1) dah <- expm1(dat.d) #deshacer transformaci?n logar?tmica
  else if(trf>1) dah <- dat.d^trf #deshacer transformaci?n ra?z
  else dah <- dat.d
  dah <- round(dah,ndec) #redondear con el no. de decimales deseado
  #- gr?ficos de las series homogeneizadas y sus correcciones
  if(gp>2) {
    par(old.par)
    plot(-1:1,-1:1,type="n",xaxt="n",yaxt="n",bty="n",xlab="",ylab="")
    ...
    for(i in 1:nei) { #para cada estaci?n original
      wi <- which(iest==i) #estaciones derivadas de la estaci?n i
      lwi <- length(wi)
      if(!lwi) next #(estaci?n totalmente borrada!)
  ...
  cat("\n----------- Final computations:\n")
  #autocorrelaciones de las anomal?as de cada estaci?n
  cat("\nACmx: Station maximum absolute autocorrelations of anomalies\n")
  sac <- rep(NA,ne) #vector de m?ximas autocorrelaciones
  for(i in 1:ne) {
    zz <- acf(anom[,i],plot=FALSE,na.action=na.pass)$acf
    zz[1] <- 0 #anulamos la autocorrelaci?n trivial
    sac[i] <- max(abs(zz)) #m?xima autocorrelaci?n con diferentes desfases
  }
  print(summary(round(sac,2)))
  #prueba SNHT de cada estaci?n
  cat("\nSNHT: Standard normal homogeneity test (on anomaly series)\n")
  print(summary(round(snhx,1)))
  #errores t?picos de las estimas (sin estandarizar):
  ...
  #averiguar qu? estaciones derivadas funcionan al final del periodo:
  cur <- apply(!is.na(dat[(nd-mndat+1):nd,]),2,sum) #?ltimos mndat t?rminos
  cur[cur>0] <- 1
  #a?adir cinco nuevas columnas a la tabla de estaciones (porcentaje de datos
  #originales, estaci?n original, si funciona actualmente, SNHT y RMSE):
  est.c <- cbind(est.c,pod,iest,cur,snhx,rmse)
  #- if(gp>1), ?ltimos gr?ficos (hist. de anomal?as y snht; calidad/singular.)
  if(gp>1) {
    #histograma de las anomal?as (las de los outliers, en rojo):
...
#     barplot(zy,log='y',space=0,col=hsv(0,.75),add=TRUE,ylim=c(.1,ymax))
    ...
    if(sum(!is.na(z))) hist(z,breaks=20,xlab="SNHT",col="purple",main=main)
    #gr?fico de calidad/singularidad:
  ...
  if(gp>0) { par(old.par); graphics.off() } #cerrar la salida gr?fica
  ...
  if(nelim>0) { #si se eliminaron estaciones:
    #ajustar los ?ndices de estaci?n original:
    ...
    est.c[,7] <- iest #actualizar los ?ndices de estaci?n original
...
#nmt: no. m?nimo de t?rminos de cada muestra
  n <- length(x)
  Tsnht <- rep(NA,n)
# Ssnht <- rep(NA,n)
  if(n<nmt*2) return(Tsnht) #insuficientes datos
  z <- (x-mean(x,na.rm=TRUE))/sd(x,na.rm=TRUE)
  for(i in (nmt+1):(n-nmt)) { #(despreciar los primeros y ?ltimos nmt t?rminos)
    if(is.na(x[i])) next
    n1 <- sum(!is.na(x[1:(i-1)])) #no. de t?rminos de la muestra 1
    n2 <- sum(!is.na(x[i:n])) #no. de t?rminos de la muestra 2
    if(n1<nmt | n2<nmt) next #al menos una muestra es demasiado peque?a
...
#- snhtw.- SNHT para ventanas solapadas de 2*nt t?rminos v?lidos.
snhtw <- function(x,nt=48) {
  ntt <- length(x) #no. total de t?rminos de la serie
  ntv <- sum(!is.na(x)) #no. de t?rminos v?lidos de la serie
  if(2*nt>ntv) return(c(0,0)) #no hay suficientes datos v?lidos para la prueba
  tV <- 0 #inicializaci?n del tV m?ximo a devolver
  pk <- 0 #inicializaci?n de la posici?n a devolver
  #inicializaci?n de los l?mites muestrales (a1-b1, a2-b2):
  ...
  n<-1; while(n<nt & k<ntt) { k <- k+1; if(!is.na(x[k])) n <- n+1; }
  b2 <- k
  #aplicaci?n de SNHT a las ventanas solapadas:
  repeat {
    st <- snht(x[a1:b2])
